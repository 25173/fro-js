...
var part4 = {
    ...
    instruction1: function () {
        ...
        var Yosef = {naam:'Yosef', groep:'MD2', lessons: ["FRO", "BAP", "PROJ"] };
        var result = "Student "+ Yosef.naam + " East zit in groep "+ Yosef.groep +" en zijn eerste les is "+ Yosef.lessons[0];
    ...
    instruction2: function () {
        ...
        // Zorg ervoor dat de opdracht goed wordt uitgevoerd en dat de content wordt bijgewerkt.
        // Converteer dit nu naar JSON
        var lines = studentsArray;
        // read column names
    ...
    instruction3: function () {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                var result = "<h4>JSON-formaat</h4>";
                result += addLine(this.responseText);
                var myArr = JSON.parse(this.responseText).people;
                result += "<h4>Mooi lijstje</h4>";
                result += "<ul>";
                for (var i = 0; i < myArr.length; i++) {
                    result += "<li>" + myArr[i].name + "</li>";
                }
                result += "</ul>";

                updateContent("Opdracht 14 - Publieke webAPI aanroepen", description, result);
            }
        };
    ...
    instruction4: function () {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                updateContent("Opdracht 15 - eigen webAPI aanroepen", description, result);
            }
        };
        xhttp.open("GET", url, true);
        xhttp.send();
